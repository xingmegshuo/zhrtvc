...
from synthesizer.audio import load_wav, melspectrogram, linearspectrogram
...
class Feeder:
	...
	def __init__(self, coordinator, metadata_filename, hparams):
		...
		with open(metadata_filename, encoding="utf-8") as f:
			self._metadata = [line.strip().split("|") for line in tqdm(f)]
	...
	def _get_test_groups(self):
		...
		embed_target = np.load(meta[2])  # os.path.join(self._embed_dir, meta[2])
